...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Proveri da li je fajl stariji od N dana."
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Prag u danima (podrazumevano: 7)."
    )
    return p.parse_args()
...
def main() -> int:
    ...
    print(f"[🕓] Last modified: {mtime.strftime('%Y-%m-%d %H:%M:%S')} ({human_delta(mtime, now)})")
    print(f"[🗓️] Threshold: {threshold.strftime('%Y-%m-%d %H:%M:%S')} (>{args.days} days old)")
