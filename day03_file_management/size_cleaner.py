...
def clean_by_size(folder_path, min_size=100):
    ...
    """
    try:
        log(f"Započinjem čišćenje fajlova manjih od {min_size} bajtova u folderu: {folder_path}", level="INFO")
        deleted_files = 0
            ...
            for file in files:
                ...
                try:
                    ...
                    if file_size < min_size:
                        os.remove(file_path)
                        deleted_files += 1
                        log(f"Obrisan fajl: {file_path} ({file_size} bajtova)", level="INFO")
                except Exception as e:
                    log(f"Greška pri proveri fajla {file_path}: {str(e)}", level="WARNING")

        log(f"Čišćenje završeno. Ukupno obrisano {deleted_files} fajlova.", level="INFO")
    except Exception as e:
